            ...
            : base(
                  x => x.User.Any(y => y.Id == filterModel.UserId)
                        && (!x.StudentPublicationReport.Any(y => y.UserId == filterModel.UserId && (y.State == ReportState.Signed || y.State == ReportState.Confirmed))
                            || x.RecomendedPublicationReport.Any(y => y.UserId == filterModel.UserId && (y.State == ReportState.Signed || y.State == ReportState.Confirmed))
                            || x.AcceptedToPrintPublicationReport.Any(y => y.UserId == filterModel.UserId && (y.State == ReportState.Signed || y.State == ReportState.Confirmed)))
